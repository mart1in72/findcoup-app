...
def main(page: ft.Page):
    ...
    def route_change(route):
        ...
        if page.route == "/":
            ...
            def login_process(_):
                ...
                if res and res.data:
                    ...
                    if str(db_u.get("password")) == password:
                        ...
                        ps_field.error_text = "Ошибка пароля"; page.update()
                else:
                    un_field.error_text = "Не найден"; page.update()
            ...
            page.views.append(ft.View("/", [
                ...
                ft.TextButton("Забыли пароль?", on_click=lambda _: page.go("/reset_password"), style=ft.ButtonStyle(color="grey"))
        ...
        # --- ЭКРАН: СБРОС ПАРОЛЯ ---
            ...
            def do_reset(_):
                ...
                check = safe_query(lambda: supabase.table("profiles").select("username").eq("username", rs_un.value).execute())
                if check and check.data:
                    safe_query(lambda: supabase.table("profiles").update({"password": rs_ps.value}).eq("username", rs_un.value).execute())
                    show_msg("Готово!", ft.Colors.GREEN_700)
                    page.go("/")
                else: show_msg("Ник не найден")
        ...
        # --- ЭКРАН: РЕГИСТРАЦИЯ ---
            ...
            avatar_preview = ft.CircleAvatar(radius=60, bgcolor=ft.Colors.GREY_900, content=ft.Icon(ft.Icons.ADD_A_PHOTO))
            ...
            def do_reg(_):
                if not r_un.value or not reg_temp_avatar_url:
                    show_msg("Нужны ник и фото!"); return
        ...
        # --- ЭКРАН: ЛЕНТА ЗНАКОМСТВ ---
            ...
            def next_p(_=None):
                ...
                if res and res.data:
                    ...
                    card_stack.data = p
                    card_stack.controls = [
                        ft.Container(ft.Image(src=p.get("avatar_url"), fit="cover"), border_radius=30, height=480, width=360),
                        ft.Text(f"{p['first_name']}", size=28, weight="bold"),
                        ft.Text(p.get("bio", ""), italic=True, color="grey")
                    ]
                else: card_stack.controls = [ft.Text("Никого нет рядом", color="grey")]
                page.update()

            def do_like(_):
                if not card_stack.data: return
                safe_query(lambda: supabase.table("likes").insert({"from_email": user_state["email"], "to_email": card_stack.data['email']}).execute())
                next_p()
        ...
        # --- ЭКРАН: МЭТЧИ ---
            ...
            likes = safe_query(lambda: supabase.table("likes").select("to_email").eq("from_email", user_state["email"]).execute())
            if likes and likes.data:
                for l in likes.data:
                    mut = safe_query(lambda: supabase.table("likes").select("*").eq("from_email", l['to_email']).eq("to_email", user_state["email"]).execute())
                    if mut and mut.data:
                        p_res = safe_query(lambda: supabase.table("profiles").select("*").eq("email", l['to_email']).single().execute())
                        if p_res:
                            u = p_res.data
            ...
            page.views.append(ft.View("/matches", [ft.AppBar(title=ft.Text("Мэтчи")), get_nav(1), m_list], bgcolor="black"))

        # --- ЭКРАН: СПИСОК ЧАТОВ ---
        elif page.route == "/messages":
            c_list = ft.ListView(expand=True, padding=10)
            raw = safe_query(lambda: supabase.table("messages").select("sender_email, receiver_email").or_(f"sender_email.eq.{user_state['email']},receiver_email.eq.{user_state['email']}").execute())
            emails = {m['receiver_email'] if m['sender_email'] == user_state['email'] else m['sender_email'] for m in raw.data} if raw else set()
            for em in emails:
            ...
            page.views.append(ft.View("/messages", [ft.AppBar(title=ft.Text("Чаты")), get_nav(2), c_list], bgcolor="black"))
        ...
        # --- ЭКРАН: ОКНО ЧАТА ---
            ...
            def sync_chat():
                ...
                if res and res.data:
                    msg_view.controls = [ft.Row([ft.Container(content=ft.Text(m['text']), bgcolor="red" if m['sender_email'] == user_state['email'] else "#222222", padding=12, border_radius=15)], alignment=ft.MainAxisAlignment.END if m['sender_email'] == user_state['email'] else ft.MainAxisAlignment.START) for m in res.data]
                    page.update()

            def poll():
                while chat_active:
                    time.sleep(4); sync_chat()
        ...
        # --- ЭКРАН: ПРОФИЛЬ ---
            ...
            def save_p(_):
                safe_query(lambda: supabase.table("profiles").update({"first_name": ed_fn.value, "bio": ed_bio.value}).eq("email", user_state["email"]).execute())
    ...
    def open_chat(u):
        current_chat_partner.update({"email": u['email'], "username": u['username'], "avatar_url": u.get("avatar_url", "")})
        page.go("/chat")

    page.on_route_change = route_change
    # page.go("/") # Заменено на прямой вызов для корректной инициализации в браузере
